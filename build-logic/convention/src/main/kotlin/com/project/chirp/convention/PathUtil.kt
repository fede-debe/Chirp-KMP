 ...
 *
 * ## Strategy / Decisions
 * - **Dynamic Convention over Configuration:** Instead of hardcoding namespaces (e.g., `com.project.chat.data`) or resource prefixes in every single module's `build.gradle.kts`, we use the module's absolute Gradle path (e.g., `:core:data`) to derive these values automatically. This heavily reduces boilerplate and centralizes naming rules across all modules.
 *
 * ## How It Works
 * 1. **pathToPackageName:** Takes the module path, replaces colons with periods, converts to lowercase, and prepends the root project package name (e.g., `:core:data` -> `com.project.core.data`).
