...
/**
 * Centralizes the application's raw branding color palette and defines custom color tokens.
 *
 * ## Strategy / Decisions
 * We separate the definition of raw hexadecimal colors from their theme assignments.
 * Because Material 3's default color tokens (primary, secondary, background, etc.) are insufficient
 * for the complexities of the Figma designs (which include custom states like "primary hover" and
 * specific elements like 10+ chat bubble color variations), we model an `ExtendedColors`
 * data class to hold these non-standard design tokens.
 *
 * ## How It Works
 * 1. Defines all raw colors as top-level properties utilizing Compose hexadecimal `0xFF...` formatting.
 * 2. Groups all non-standard Material 3 color tokens into a standalone `ExtendedColors` data class.
 *
 * ## Technical Details
 * - The `ExtendedColors` data class is annotated with `@Immutable`. This explicitly tells the Compose
 *   compiler that the class properties will never change internally. If a change occurs (e.g., a theme switch),
 *   the entire instance is replaced. This enables aggressive recomposition optimizations.
 */
// Brand colors
